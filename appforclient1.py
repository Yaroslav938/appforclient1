...
# ─────────────────────────────────────────────
# ПАРСЕР ПО ССЫЛКАМ ЧЕРЕЗ META-ТЕГИ И YT-DLP
# ─────────────────────────────────────────────
def parse_ig_urls(urls):
    ...
    # ХИТРОСТЬ: Притворяемся сервером Facebook/WhatsApp.
    # Инстаграм добровольно отдаст нам лайки, комменты и текст для "генерации превью ссылки".
    headers = {
    ...
    for idx, url in enumerate(urls):
        ...
        # ПОПЫТКА 1: Сбор через мета-теги (Обходит блокировки Streamlit Cloud)
        try:
            r = requests.get(url, headers=headers, timeout=10)
            if r.status_code == 200:
                # Ищем тег og:description, в котором зашита статистика
                desc_match = re.search(r'<meta\s+property="og:description"\s+content="(.*?)"', r.text)
                if desc_match:
                    content = html.unescape(desc_match.group(1))
                    ...
                    # Парсим "45K Likes"
                    likes_m = re.search(r'([\d,\.]+[KMB]*)\s+Likes', content, re.IGNORECASE)
                    if likes_m:
                        val = likes_m.group(1).upper().replace(',', '')
                        if 'K' in val: likes = int(float(val.replace('K','')) * 1000)
                        elif 'M' in val: likes = int(float(val.replace('M','')) * 1000000)
                        else: likes = int(float(val))
                        
                    # Парсим "1,200 Comments"
                    comm_m = re.search(r'([\d,\.]+[KMB]*)\s+Comments', content, re.IGNORECASE)
                    if comm_m:
                        val = comm_m.group(1).upper().replace(',', '')
                        if 'K' in val: comments = int(float(val.replace('K','')) * 1000)
                        elif 'M' in val: comments = int(float(val.replace('M','')) * 1000000)
                        else: comments = int(float(val))
                        
                    # Парсим автора
                    author_m = re.search(r'(@[\w\._]+)', content)
                    if author_m:
                        author = author_m.group(1)
                        
                    # Парсим текст Reels
                    caption_m = re.search(r'Instagram:\s*"(.*?)"', content, re.DOTALL)
                    if caption_m:
                        caption = caption_m.group(1)
                    else:
                        caption = content
                        
                    # Если нашли статистику или автора, сохраняем
                    if likes > 0 or author != "Неизвестно":
                        views = likes * 12 # Восстанавливаем просмотры
                        days_ago = 14 # Оценка, так как даты в OG нет
        ...
        # ПОПЫТКА 2: Фолбэк на yt-dlp (Если 1 способ не сработал)
        if not success:
            try:
                import yt_dlp
...
with tab3:
    ...
    if not st.session_state.openai_key:
        st.warning("⚠️ Введите API-ключ OpenAI в боковой панели (слева), чтобы генерировать тексты автоматически прямо внутри этой программы.")
        st.info("💡 **Нет API-ключа? Не проблема!** Выберите видео ниже, и программа создаст идеальный Промпт (запрос). Просто скопируйте его и вставьте в бесплатный ChatGPT.")
        
        if not df.empty:
            st.markdown("### 1. Выберите референс")
            ref_options = [f"[{row['score']}/100] {row['author']} - 👁 {fmt_number(row['views'])} - {row['caption'][:50]}..." for _, row in df.iterrows()]
            selected_ref = st.selectbox("Видео для адаптации (ручной режим):", ref_options)
            
            ref_idx = ref_options.index(selected_ref)
            top_vid = df.iloc[ref_idx]
            
            st.markdown("### 2. Скопируйте этот текст в бесплатный ChatGPT")
...
Мой оффер: бесплатная диагностика и быстрый ремонт.

Сделай по структуре:
1. Хук (0-3 сек) — цепляющая фраза
2. Визуал (что конкретно показывать в кадре)
3. Озвучка (динамичный текст)
4. Призыв к действию (подписка или приглашение в сервис)""", language="text")
